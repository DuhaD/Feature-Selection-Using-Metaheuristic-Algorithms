...
filename = strcat('WithFilterFeatureSelectionChi2HBO',num2str(MaxIteration), '_', num2str(AgentsNum));
...
for i=1:8 %8 datasets
    ...
    for algo = 1: 4 %4 algorithms 
        ...
        for j=1:10 % 10 runs for each dataset 
            ...
            [training_selected_f, testing_selected_f, feature_selected_f ] = chi2(40,training_data,training_label,testing_data, features(:,1:end-1));
            
            dataset.nvars = size(training_selected_f,2);
            dataset.data = training_selected_f;
            dataset.lable = training_label;
            
            %callmeta
              [ Best_score , generations_best, individual , initime] = heapbased(dataset,MaxIteration,AgentsNum);
             
                individual_01=individual>=0.5;
               individual_01=logical(individual_01);

              training_selected = training_selected_f(:,individual_01 == 1);
              testing_selected = testing_selected_f(:, individual_01 == 1);
              feature_selected = feature_selected_f(:, individual_01 == 1);
